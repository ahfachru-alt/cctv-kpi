...
<div>
    ...
    <form wire:submit="login">
        ...
        <div class="flex items-center justify-between mt-6">
            <form method="POST" action="{{ route('google.token') }}" id="google-login-form" class="hidden">
                @csrf
                <input type="hidden" name="credential" id="google-credential" />
            </form>
            <button type="button" id="google-login-btn" class="inline-flex items-center gap-2 rounded-md bg-red-600 text-white px-3 py-2 hover:bg-red-700 transition">
        ...
        window.addEventListener('load', () => {
            ...
            btn?.addEventListener('click', () => {
                google.accounts.id.initialize({ client_id: clientId, callback: (resp) => {
                    document.getElementById('google-credential').value = resp.credential;

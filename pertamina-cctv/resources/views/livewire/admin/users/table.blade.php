<div class="p-4" x-data="{ delId: null }">
	<div class="flex items-center justify-between mb-3">
		...
		<a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-1 px-3 py-2 rounded-md font-medium text-white bg-indigo-600 hover:bg-indigo-700">
			...
			<tbody>
				...
				<tr class="border-t border-gray-200 dark:border-gray-700">
					...
					<td class="px-3 py-2 text-right flex gap-2 justify-end">
						<a href="{{ route('admin.users.edit',$u) }}" class="inline-flex items-center gap-1 px-2 py-1 rounded bg-gray-200 dark:bg-gray-700">
						...
						<button type="button" @click="delId={{ $u->id }}" class="inline-flex items-center gap-1 px-2 py-1 rounded bg-red-600 text-white">
	...
	<div x-show="delId" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4" x-cloak>
		<div class="bg-white dark:bg-gray-800 rounded shadow p-4 w-full max-w-sm">
			...
			<div class="flex justify-end gap-2">
				...
				<button type="button" class="px-3 py-2 rounded bg-red-600 text-white" @click="$wire.emit('deleteUser', delId); delId=null">Hapus</button>
			</div>
		</div>
	</div>
</div>

<div>
	<form method="POST" action="{{ route('register') }}">
		...
		<div class="flex items-center justify-between mt-6">
			<form method="POST" action="{{ route('google.token') }}" id="admin-google-register-form" class="hidden">
				@csrf
				<input type="hidden" name="credential" id="admin-google-register-credential" />
			</form>
			<button type="button" id="admin-google-register-btn" class="inline-flex items-center gap-2 rounded-md bg-red-600 text-white px-3 py-2 hover:bg-red-700 transition">
	...
	<script>
		window.addEventListener('load', () => {
			...
			const btn = document.getElementById('admin-google-register-btn');
			btn?.addEventListener('click', () => {
				google.accounts.id.initialize({ client_id: clientId, callback: (resp) => {
					document.getElementById('admin-google-register-credential').value = resp.credential;
					document.getElementById('admin-google-register-form').submit();
				}});
